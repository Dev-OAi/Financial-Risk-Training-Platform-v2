...
export const SearchBox: React.FC<SearchBoxProps> = ({ onSearch, isGenerating, themeMode }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-sm md:max-w-md relative flex items-center">
      <div className={`absolute left-3 ${themeMode === 'dark' ? 'text-[#bdc1c6]' : 'text-[#5f6368]'} font-semibold flex items-center gap-1.5 pointer-events-none`}>
      ...
      <input
        ...
        className={`w-full pl-8 md:pl-12 pr-10 py-1.5  border text-xs shadow-sm focus:outline-none focus:ring-1 focus:ring-[#5f6368] transition-all ${
          themeMode === 'dark'
      ...
      <button
        ...
        className="absolute right-1.5 p-1  bg-[#3c4043] hover:bg-[#5f6368] text-[#e8eaed] shadow-xs disabled:opacity-50 disabled:cursor-not-allowed transition-all"
