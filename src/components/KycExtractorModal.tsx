/**
 * @file KycExtractorModal.tsx
 * @description Driver's License KYC Data & Expiry Extractor modal for digital & branch account opening.
 */

import React, { useState } from 'react';
import { X, Upload, CheckCircle2, AlertTriangle, ShieldCheck, FileText, Cpu, UserCheck, Calendar } from 'lucide-react';
import { ThemeMode } from '../types';

interface KycExtractorModalProps {
  isOpen: boolean;
  onClose: () => void;
  themeMode: ThemeMode;
}

export const KycExtractorModal: React.FC<KycExtractorModalProps> = ({
  isOpen,
  onClose,
  themeMode
}) => {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [isProcessing, setIsProcessing] = useState<boolean>(false);
  const [extractedData, setExtractedData] = useState<any | null>(null);

  if (!isOpen) return null;

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setSelectedFile(file);
      const reader = new FileReader();
      reader.onloadend = () => {
        setPreviewUrl(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };

  const handleRunKycExtraction = async () => {
    if (!previewUrl) return;
    setIsProcessing(true);
    
    try {
      // Simulate or call OCR vision for KYC driver's license extraction
      setTimeout(() => {
        const todayStr = new Date().toISOString().split('T')[0];
        const mockResult = {
          fullLegalName: "Evelyn Marie Vance",
          address: "742 Evergreen Terrace, Springfield, IL 62704",
          dateOfBirth: "1988-04-14",
          licenseNumber: "V650-882-34-109-0",
          issueDate: "2021-04-14",
          expirationDate: "2027-04-14",
          todayDate: todayStr,
          isExpired: false,
          verificationStatus: "VERIFIED_ACTIVE",
          confidenceScore: 98.4,
          securityFeaturesDetected: ["Ghost Image Hologram", "Micro-print Border", "UV State Seal"]
        };
        setExtractedData(mockResult);
        setIsProcessing(false);
      }, 1200);
    } catch (err) {
      setIsProcessing(false);
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-4">
      <div className={`w-full max-w-3xl rounded-2xl border shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${
        themeMode === 'dark' ? 'bg-[#202124] border-[#3c4043] text-[#e8eaed]' : 'bg-white border-slate-200 text-slate-900'
      }`}>
        {/* Header */}
        <div className={`px-6 py-4 border-b flex items-center justify-between ${
          themeMode === 'dark' ? 'border-[#3c4043] bg-[#2d2e31]' : 'border-slate-200 bg-slate-50'
        }`}>
          <div className="flex items-center gap-2.5">
            <UserCheck className="w-5 h-5 text-emerald-500" />
            <div>
              <h2 className="font-bold text-sm uppercase tracking-wider">Driver's License KYC Data & Expiry Extractor</h2>
              <p className="text-xs opacity-75">Automated Branch & Digital Account Opening Identity Verification</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg opacity-75 hover:opacity-100 hover:bg-black/10 transition">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Content Body */}
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Upload Box */}
            <div className="space-y-4">
              <label className="block text-xs font-semibold uppercase tracking-wider opacity-80">
                Upload State Driver's License / ID Photo
              </label>

              {!previewUrl ? (
                <label className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer transition ${
                  themeMode === 'dark' ? 'border-[#5f6368] hover:border-emerald-400 bg-[#292a2d]' : 'border-slate-300 hover:border-emerald-600 bg-slate-50'
                }`}>
                  <Upload className="w-8 h-8 text-emerald-500 mb-2" />
                  <span className="text-xs font-medium text-center">Click to browse or drop ID image</span>
                  <span className="text-[10px] opacity-60 mt-1">Supports PNG, JPG, WEBP (Front ID)</span>
                  <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                </label>
              ) : (
                <div className="relative rounded-xl overflow-hidden border border-inherit bg-black/40 p-2 flex flex-col items-center">
                  <img src={previewUrl} alt="ID Preview" className="max-h-[220px] object-contain rounded" />
                  <button
                    onClick={() => { setPreviewUrl(null); setSelectedFile(null); setExtractedData(null); }}
                    className="mt-3 px-3 py-1 rounded bg-rose-600/80 hover:bg-rose-600 text-white text-xs font-medium transition"
                  >
                    Remove / Change Photo
                  </button>
                </div>
              )}

              {previewUrl && !extractedData && (
                <button
                  onClick={handleRunKycExtraction}
                  disabled={isProcessing}
                  className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow flex items-center justify-center gap-2 transition disabled:opacity-50"
                >
                  {isProcessing ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                      <span>Extracting KYC Fields & Expiry...</span>
                    </>
                  ) : (
                    <>
                      <Cpu className="w-4 h-4" />
                      <span>Run AI Vision KYC Extractor</span>
                    </>
                  )}
                </button>
              )}
            </div>

            {/* Extracted JSON / Results Panel */}
            <div className="space-y-3 flex flex-col">
              <label className="block text-xs font-semibold uppercase tracking-wider opacity-80">
                Structured KYC Extraction & Expiry Verification
              </label>

              <div className={`flex-1 rounded-xl p-4 font-mono text-xs border overflow-y-auto ${
                themeMode === 'dark' ? 'bg-[#18191c] border-[#3c4043]' : 'bg-slate-900 text-slate-100 border-slate-800'
              }`}>
                {extractedData ? (
                  <pre className="text-[11px] leading-relaxed text-emerald-300">
                    {JSON.stringify(extractedData, null, 2)}
                  </pre>
                ) : (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-50 p-6 space-y-2">
                    <FileText className="w-8 h-8" />
                    <p className="text-xs">Upload an ID photo and click Run AI Vision KYC Extractor to generate structured JSON.</p>
                  </div>
                )}
              </div>
            </div>
          </div>

          {extractedData && (
            <div className={`p-4 rounded-xl border flex items-center justify-between ${
              extractedData.isExpired 
                ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' 
                : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
            }`}>
              <div className="flex items-center gap-3">
                {extractedData.isExpired ? <AlertTriangle className="w-5 h-5 shrink-0" /> : <CheckCircle2 className="w-5 h-5 shrink-0" />}
                <div>
                  <div className="font-bold text-xs uppercase tracking-wider">
                    {extractedData.isExpired ? 'ID Expired - Verification Failed' : 'KYC Expiration Verified Active'}
                  </div>
                  <div className="text-[11px] opacity-90">
                    Expiration Date: {extractedData.expirationDate} (Compared against Today: {extractedData.todayDate})
                  </div>
                </div>
              </div>
              <span className="text-xs font-bold px-2.5 py-1 rounded bg-black/20">
                {extractedData.confidenceScore}% Confidence
              </span>
            </div>
          )}
        </div>

        {/* Footer */}
        <div className={`px-6 py-3 border-t flex justify-end ${
          themeMode === 'dark' ? 'border-[#3c4043] bg-[#2d2e31]' : 'border-slate-200 bg-slate-50'
        }`}>
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs shadow transition"
          >
            Close Extractor
          </button>
        </div>
      </div>
    </div>
  );
};
